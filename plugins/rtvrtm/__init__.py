...
class RTVRTMPlugin:
    ...
    def setup_rtvrtm_files(self):
        ...
        try:
            # Get MBII folder path from config
            mbii_folder = self.config.get('general', {}).get('mbii_folder', '/opt/openjk/MBII')
            
            # Create file names with instance prefix
            instance_name = self.instance.settings.get('instance_name', 'default')
...
""".format(
            # General settings
            log=self.config.get('general', {}).get('log', ''),
            mbii_folder=self.config.get('general', {}).get('mbii_folder', ''),
            address=self.config.get('general', {}).get('address', ''),
            bind=self.config.get('general', {}).get('bind', ''),
            password=self.config.get('general', {}).get('password', ''),
    ...
    def start_rtvrtm(self):
        ...
        def run_rtvrtm():
            try:
                ...
                rtvrtm_script = os.path.join(plugin_dir, 'rtvrtm.py')
