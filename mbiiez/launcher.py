    ...
    def openjk_launch(self):   
        ...
        while(True):
            ...
            while(not self.process_handler.process_status("OpenJK")): 
                ...
                cmd = "nohup {} --quiet +set dedicated 2 +set net_port {} +set fs_game {}{} +exec {}".format(self.config['server']['engine'], self.config['server']['port'], settings.dedicated.game, self.instance.get_startup_cvar_args(), self.config['server']['server_config_path']);       
